"""
    ...
    Gestions des "routes" FLASK et des données pour l'association entre les pationt et les medecin.
"""
...
"""
    ...
    But : Afficher les pationt avec les medecin associés pour chaque film.
    
    Paramètres : id_genre_sel = 0 >> tous les pationt.
...
def films_genres_afficher(id_film_sel):
    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                ...
                #                                             GROUP BY id_film"""
                strsql_genres_films_afficher_data = """SELECT  p.id_pationt,  p.nom_pationt,  p.prenome_pationt,  p.date_naissance, p.adresse, p.telephone, p.email, f.id_facture, f.montant, f.date_emission, f.fk_consultation, f.fk_pationt
                                                        FROM t_pationt p
                                                        LEFT JOIN t_facture f ON p.id_pationt = f.fk_pationt;"""
                if id_film_sel == 0:
                    # le paramètre 0 permet d'afficher tous les pationt
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    ...
                    flash(f"Données pationt et medecin affichés !!", "success")
    ...
    return render_template("facture/films_genres_afficher.html", data=data_genres_films_afficher)
...
"""
    ...
    Récupère la liste de tous les medecin du film sélectionné par le bouton "MODIFIER" de "films_genres_afficher.html"
    
    Dans une liste déroulante particulière (tags-selector-tagselect), on voit :
    1) Tous les medecin contenus dans la "t_genre".
    2) Les medecin attribués au film selectionné.
    3) Les medecin non-attribués au film sélectionné.
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT id_pationt, nom_pationt, prenome_pationt FROM t_pationt ORDER BY id_pationt ASC"""
            ...
            # 2) Sélection des medecin "déjà" attribués pour le film.
            # 3) Sélection des medecin "pas encore" attribués pour le film choisi.
            ...
            # les medecin qui ne sont pas encore sélectionnés.
            ...
            # Dans le composant "tags-selector-tagselect" on doit connaître
            # les medecin qui sont déjà sélectionnés.
    ...
    return render_template("facture/films_genres_modifier_tags_dropbox.html",
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            # Récupère la liste des medecin qui ne sont pas associés au film sélectionné.
            ...
            # Récupère la liste des medecin qui sont associés au film sélectionné.
            ...
            # Récupère ce que l'utilisateur veut modifier comme medecin dans le composant "tags-selector-tagselect"
            ...
            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des medecin à INSÉRER dans la "t_genre_film".
                ...
                # Pour le film sélectionné, parcourir la liste des medecin à EFFACER dans la "t_genre_film".
                # Si la liste est vide, la boucle n'est pas parcourue.
    ...
    # on affiche les pationt et le(urs) genre(s) associé(s).
...
"""
    ...
    Récupère la liste de tous les medecin du film sélectionné par le bouton "MODIFIER" de "films_genres_afficher.html"
    Nécessaire pour afficher tous les "TAGS" des medecin, ainsi l'utilisateur voit les medecin à disposition
