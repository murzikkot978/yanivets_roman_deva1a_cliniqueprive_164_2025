"""Gestion des "routes" FLASK et des données pour les pationt.
...
Remarque :  Dans le champ "nom_film_update_wtf" du formulaire "pationt/films_update_wtf.html",
...
def film_add_wtf():
    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                # Pour afficher et constater l'insertion du nouveau film (id_film_sel=0 => afficher tous les pationt)
    ...
    return render_template("pationt/film_add_wtf.html", form_add_film=form_add_film)
...
def film_update_wtf():
    ...
    try:
        ...
        if request.method == "POST" and form_update_film.submit.data:
            ...
            str_sql_id_film = "SELECT * FROM t_pationt;"
    ...
    return render_template("pationt/film_update_wtf.html", form_update_film=form_update_film)
...
Remarque :  Dans le champ "nom_film_delete_wtf" du formulaire "pationt/film_delete_wtf.html"
...
def film_delete_wtf():
    ...
    try:
        # Si on clique sur "ANNULER", afficher tous les pationt.
        if form_delete_film.submit_btn_annuler.data:
            return redirect(url_for("films_genres_afficher", id_film_sel=0))

        if form_delete_film.submit_btn_conf_del_film.data:
            # Récupère les données afin d'afficher à nouveau
            # le formulaire "pationt/film_delete_wtf.html" lorsque le bouton "Etes-vous sur d'effacer ?" est cliqué.
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_pationt WHERE id_pationt = %(value_id_pationt)s"""
    ...
    return render_template("pationt/film_delete_wtf.html",
